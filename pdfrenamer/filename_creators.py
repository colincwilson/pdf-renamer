...
AllowedTags = {
    ...
    "{Jabbr}":
    " \t=\t Abbreviated name of Journal, if any available (otherwise full name is used)",
    "{Aall}": " \t\t=\t Last name of all authors (separated by comma)",
    "{Aetal}":
    " \t=\t Last name of the first author, add \'et al.\' if more authors are present",
    "{A3etal}":
    " \t=\t Last name of the first three authors (separated by comma), add \'et al.\' if more authors are present",
    "{aAall}":
    " \t=\t First initial and last name of all authors (separated by comma)",
    "{aAetal}":
    " \t=\t First initial and last name of the first author, add \'et al.\' if more authors are present",
    "{aA3etal}":
    " \t=\t First initial and last name of the first three authors (separated by comma), add \'et al.\' if more authors are present",
    "{T}": " \t\t=\t Title"
...
def sanitize(string):
    ...
    replace = {
        ...
        '}': ["{\\textbraceright}"],
        "\'": [
            "{\\textquotesingle}", "{\\textquotedblleft}",
...
def find_abbreviation_journal(journal_name):
    ...
    data = pkgutil.get_data(__name__,
                            "UserDefinedAbbreviations.txt").decode('utf8')
    for line in data.splitlines():
        if (line.lower()).startswith(to_search):
            return line[len(to_search):].rstrip()

    data = pkgutil.get_data(__name__,
                            "StandardAbbreviations.txt").decode('utf8')
    for line in data.splitlines():
        if (line.lower()).startswith(to_search):
            return line[len(to_search):].rstrip()
    return None
...
def build_filename(infos, format=None, tags=None):
    ...
    if '{YYYY}' in rep_dict.keys():
        rep_dict['{YYYY}'] = infos['year'] if (
            'year' in infos and is_valid_integer(infos['year'], 4)) else '0000'
    ...
    ListAuthorTags = [
        '{Aall}', '{A3etal}', '{Aetal}', '{aAall}', '{aA3etal}', '{aAetal}'
    ]
    author_sep = config.get('author_sep')
    author_etal = config.get('author_etal')
    if any(
            item in rep_dict.keys() for item in ListAuthorTags
    ):  #Check if any of the tag chosen by the user is one of the author tags defined in ListAuthorTags
        if author_info:
            ...
            if isinstance(author_info, list):
                lastnames = [
                    author['family'] for author in author_info
                    if 'family' in author
                ]
                firstnames = [
                    author['given'] for author in author_info
                    if 'given' in author
                ]
            elif isinstance(author_info, str):
                authors = [
                    author.strip() for author in author_info.split(" and ")
                ]
            ...
            if lastnames:
                rep_dict['{Aall}'] = author_sep.join(lastnames)
                rep_dict['{A3etal}'] = author_sep.join(lastnames[0:3])
                if len(lastnames) > 3:
                    rep_dict['{A3etal}'] = rep_dict['{A3etal}'] + author_etal
                rep_dict['{Aetal}'] = lastnames[0] + (
                    author_etal if len(lastnames) > 1 else "")

                if firstnames:
                    firstinitials = [
                        firstname[0][0].upper() +
                        "." if len(firstname[0]) > 0 else ""
                        for firstname in firstnames
                    ]
                    firstinitial_lastnames = [
                        firstinitials + " " + lastname
                        for (firstinitials,
                             lastname) in zip(firstinitials, lastnames)
                    ]

                    rep_dict['{aAall}'] = author_sep.join(
                        firstinitial_lastnames)
                    rep_dict['{aA3etal}'] = author_sep.join(
                        firstinitial_lastnames[0:3])
                    if len(firstinitial_lastnames) > 3:
                        rep_dict[
                            '{aA3etal}'] = rep_dict['{aA3etal}'] + author_etal
                    rep_dict['{aAetal}'] = firstinitial_lastnames[0] + (
                        author_etal if len(firstinitial_lastnames) > 1 else "")
                else:
    ...
    if (config.get('max_words_title') > 0 and 'title' in infos
    ...
    # Delete function words from space-separated title.
    if (config.get('title_nofunc') and 'title' in infos):
        func_regex = r'^(a|an|and|at|by|for|in|of|the|to|with)$'
        title_words = infos['title'].split(' ')
        title_words = [w for w in title_words if not \
            re.search(func_regex, w.lower())]
        infos['title'] = ' '.join(title_words)
    ...
    # Convert fields to specified case.
    for key in rep_dict.keys():
        if re.search('^[{][Aa]', key):  # [CW] special author_case
            if config.get('author_case') != 'none':
                rep_dict[key] = string_transformations[config.get(
                    'author_case')](rep_dict[key])
        else:
            if config.get('case') != 'none':
                rep_dict[key] = string_transformations[config.get('case')](
                    rep_dict[key])
